...
@objc class AppDelegate: FlutterAppDelegate, FlutterImplicitEngineDelegate {
  ...
  func didInitializeImplicitFlutterEngine(_ engineBridge: FlutterImplicitEngineBridge) {
    ...
    if let registrar = engineBridge.pluginRegistry.registrar(forPlugin: "BusingWidgetPlugin") {
      setupWidgetChannel(messenger: registrar.messenger())
    }
  }

  private func setupWidgetChannel(messenger: FlutterBinaryMessenger) {
    widgetChannel = FlutterMethodChannel(name: "com.example.busing/widget", binaryMessenger: messenger)
    widgetChannel?.setMethodCallHandler({
      ...
      if call.method == "checkWidgetClick" {
        ...
        if let args = call.arguments as? [String: Any] {
          ...
          if let rawItems = args["items"] as? [Any] {
            var parsedItems: [[String: Any]] = []
            for rawItem in rawItems {
              if let dict = rawItem as? [String: Any] {
                parsedItems.append(dict)
              } else if let nsDict = rawItem as? [NSObject: AnyObject] {
