...
class BusApiService {
  ...
  Future<List<BusRouteInfo>> getArrivalInfo(String busStopId, {String? stopName, String? targetBusName, double? lat, double? lng}) async {
    ...
    // 💡 45초 이내 유효한 캐시가 있다면 즉시 반환 (API 호출 90% 이상 절감 및 네트워크 딜레이 0초)
    if (_arrivalCache.containsKey(busStopId)) {
      final cache = _arrivalCache[busStopId]!;
      if (nowKst.difference(cache.timestamp).inSeconds < 45) {
    ...
    try {
      ...
      try {
        final response1 = await http.get(url).timeout(const Duration(seconds: 3));
        if (response1.statusCode == 200) items = _parseArrivalJson(response1.body);
      } catch (e) {
        debugPrint('⚠️ [BusAPI] arriveInfo(1) 예외: $e');
      }

      if (items.isEmpty) {
        try {
          final altUrl = Uri.parse(url.toString().replaceAll('BUSSTOP_ID', 'STATION_ID'));
          final response2 = await http.get(altUrl).timeout(const Duration(seconds: 3));
          if (response2.statusCode == 200) items = _parseArrivalJson(response2.body);
        } catch (e) {
          debugPrint('⚠️ [BusAPI] arriveInfo(2) 예외: $e');
        }
      }
